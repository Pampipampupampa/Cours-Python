...
class SlidePuzzle(BaseBoard):
    ...
    def __init__(self, display=pygame.Surface((640, 480)),
                 size=80, font_size=20, nb_column=4, nb_row=4, speed=None):
        ...
        # Board parameters
        self.size = size
        self.player_speed = speed or int(self.size/7)
...
class PygameMain(PygameStarter):
    ...
    def __init__(self, board, title="Slide Box Game", size=(640, 480),
                 background=(50, 60, 80), player_speed=None):
...
if __name__ == '__main__':
    main = PygameMain(SlidePuzzle(nb_column=5, nb_row=3, size=80),
                      size=(1100, 600), player_speed=40)
    main.mainloop(20)
